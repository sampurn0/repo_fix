<?php $__env->startSection('content'); ?>
	...
	<section class="grid_12">
			...
			<div class="block-border"><form class="block-content form" id="complex_form" method="post" action="#">
				<h1>Data Input Cash Processing</h1>
				<br>
					...
					<tbody>
						...
						<tr>
							...
							<td style="text-align: center">
								<button type="button" class='button green' onClick="window.location.href='<?php echo base_url();?>cashprocessing_cancel/edit/<?php echo $row->id_ct;?>#tab-hcp'" title='Edit'><span class='smaller'>View Detail Data</span></button>
							</td>
						</tr>
						<?php 
							endforeach; 
	...
	<script>
		...
		var tables = jq3412('#example').DataTable({
			...
			lengthChange: false,
			ajax: {
				url: '<?=base_url()?>cashprocessing_cancel/json',
				data: function(data) {
					// Read values
			...
			"columnDefs": [
				...
				{
					"render": function ( data, type, row ) {
						return '<button type="button" class="button green" style="font-size: 12px" onClick="window.location.href=\'<?=base_url()?>cashprocessing_cancel/edit/'+row['id_ct']+'\'" title=""><span class="">View Detail Data</span></button>';
					},
					"className": "text-center",
					"targets": 4,
					"searchable": false

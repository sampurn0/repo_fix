<?php $__env->startSection('content'); ?>
	...
	<script type="text/javascript">
			...
			$('#dg').datagrid({
				...
				onExpandRow: function(index,row){
					...
					ddv.panel({
						...
						href:'<?=base_url()?>cashprocessing_batal/show_form?index='+index+'&state='+row.state+'&act='+row.jenis+'&row='+JSON.stringify(row)+'&id='+row.id+'&id_ct='+<?=$id?>,
						onLoad:function(){
			...
			$('#dg2').datagrid({
				...
				onExpandRow: function(index,row){
					...
					ddv.panel({
						...
						cache:true,
						// href:'show_form.php?index='+index,
						href:'<?=base_url()?>cashprocessing_batal/show_form2?index='+index+'&state='+row.state+'&act='+row.jenis+'&row='+JSON.stringify(row)+'&id='+row.id+'&id_ct='+<?=$id?>,
						onLoad:function(){
		...
		function saveItemCit(index) {
			...
			var url = row.isNewRecord ? '<?=base_url()?>cashprocessing_batal/save_data_cit' : '<?=base_url()?>cashprocessing_batal/update_data_cit?id='+row.id;
			console.log(url);
		...
		function saveItem(index){
			...
			console.log(row);
			var url = row.isNewRecord ? '<?=base_url()?>cashprocessing_batal/save_data' : '<?=base_url()?>cashprocessing_batal/update_data?id='+row.id;
			console.log(url);
			...
			if (row){
				$.messager.confirm('Confirm','Are you sure you want to remove this user?',function(r){
					if (r){
						...
						console.log(row);
						$.post('<?=base_url()?>cashprocessing_batal/delete_data',{id:row.id},function(){
							$('#dg').datagrid('deleteRow',index);
						});
					}
				});
		...
		<div title="Runsheet Cashreplenish" style="padding:20px;display:none;">
			<div class="block-border">
				<form class="block-content form" id="complex_form" method="post" action="#">
					...
					<br>
					<table id="dg" class="easyui-datagrid" title="Cash Transit <?=$branch?>" style="width:100%;height:550px" data-options="rownumbers:true,singleSelect:true,url:'<?=base_url()?>cashprocessing_batal/get_data/<?=$id?>',method:'get'" toolbar="#toolbar" pagination="true">
						<thead>
							<tr>
					...
					<div id="toolbar">
						<a href="<?=base_url()?>cashprocessing_batal#&tab-hcp" class="easyui-linkbutton" iconCls="icon-back" plain="true">Back</a>
					</div>
		...
		<div title="Runsheet Cashtransit" style="padding:20px;display:none;">
			<div class="block-border">
				<form class="block-content form" id="complex_form" method="post" action="#">
					<h1>Data Input Run Sheet</h1>
					<br>
					<table id="dg2" class="easyui-datagrid" title="Cash Transit <?=$branch?>" style="width:100%;height:550px" data-options="rownumbers:true,singleSelect:true,url:'<?=base_url()?>cashprocessing_batal/get_data_cit/<?=$id?>',method:'get'" toolbar="#toolbar2" pagination="true">
						<thead>
							<tr>

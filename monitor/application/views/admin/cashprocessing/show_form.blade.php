...
<form method="post">
								...
								if(barcode.indexOf("a") != -1){
									if($("#cart_1_seal_atm_<?=$index?>").val()=="") {
										if($("#cart_1_seal_atm_<?=$index?>").val()!==barcode && 
											...
											$.post('<?php echo base_url().'cashprocessing/check_seal'?>', { value: barcode, id_bank: "<?=$row->id_bank?>" })
												.done(function( data ) {
													console.log(data);
													if(data==-1) {
														jq341.notify("Seal "+barcode+", tidak dikenal!", "error");
					...
					<div>
						...
						<table class="dv-table" style="float:left;border:0px solid #ccc;padding:5px;margin-top:5px;width:auto">
							...
							<tr>
								<td>BIG SEAL RETURN</td>
								<td>:</td>
								<td style="border: 0px; padding: 10px 15px 5px 0px"><input name="bag_seal_return" id="bag_seal_return_crm_<?=$index?>" style="height: 28px" class="easyui-validatebox easyui-textbox" required="true"></input></td>
							</tr>
							<tr>
								<td>BAG NO</td>
								<td>:</td>
					...
					<script>
						...
						jq341(document).scannerDetection({
							...
							onComplete: function(barcode, qty) {
								...
								if(barcode.indexOf("A") != -1 || barcode.indexOf("B") != -1){
									if($("#bag_seal_crm_<?=$index?>").val()=="") {
										if($("#bag_seal_crm_<?=$index?>").val()!==barcode && 
											$("#bag_seal_return_crm_<?=$index?>").val()!==barcode) {
												$.post('<?php echo base_url().'cashprocessing/check_seal'?>', { value: barcode })
													.done(function( data ) {
														console.log(data);
														if(data==-1) {
									...
									else if($("#bag_seal_return_crm_<?=$index?>").val()=="") {
										if($("#bag_seal_crm_<?=$index?>").val()!==barcode && 
											$("#bag_seal_return_crm_<?=$index?>").val()!==barcode) {
												$.post('<?php echo base_url().'cashprocessing/check_seal'?>', { value: barcode })
													.done(function( data ) {
														...
														if(data==-1) {
															...
															if(data==0) {
																...
																jq341.notify("Seal "+barcode+", siap digunakan!", "success");
																$("#bag_seal_return_crm_<?=$index?>").textbox('setValue', barcode);
															}
														}
													}
												);
					...
					<div>
						...
						<table class="dv-table" style="float:left;border:0px solid #ccc;padding:5px;margin-top:5px;width:auto">
							...
							<tr>
								<td>BIG SEAL RETURN</td>
								<td>:</td>
								<td style="border: 0px; padding: 10px 15px 5px 0px"><input name="bag_seal_return" id="bag_seal_return_cdm_<?=$index?>" style="height: 28px" class="easyui-validatebox easyui-textbox" required="true"></input></td>
							</tr>
							<tr>
								<td>BAG NO</td>
								<td>:</td>
					...
					<script>
						...
						jq341(document).scannerDetection({
							...
							onComplete: function(barcode, qty) {
								...
								if(barcode.indexOf("A") != -1 || barcode.indexOf("B") != -1){
									if($("#bag_seal_cdm_<?=$index?>").val()=="") {
										if($("#bag_seal_cdm_<?=$index?>").val()!==barcode && 
											$("#bag_seal_return_cdm_<?=$index?>").val()!==barcode) {
												$.post('<?php echo base_url().'cashprocessing/check_seal'?>', { value: barcode })
													.done(function( data ) {
														console.log(data);
														if(data==-1) {
									...
									else if($("#bag_seal_return_cdm_<?=$index?>").val()=="") {
										if($("#bag_seal_cdm_<?=$index?>").val()!==barcode && 
											$("#bag_seal_return_cdm_<?=$index?>").val()!==barcode) {
												$.post('<?php echo base_url().'cashprocessing/check_seal'?>', { value: barcode })
													.done(function( data ) {
														...
														if(data==-1) {
															...
															if(data==0) {
																...
																jq341.notify("Seal "+barcode+", siap digunakan!", "success");
																$("#bag_seal_return_cdm_<?=$index?>").textbox('setValue', barcode);
															}
														}
													}
												);
							...
							onError: function(string, qty) {
								jq341.notify("CDM "+string, "success");
								if(string=='q') {
									$.post('<?php echo base_url().'logistic_in_use/get_seal_demo'?>', { value: string, barcode: barcode })
										.done(function( data ) {
											barcode = data;
								...
								if(string=='w') {
									$.post('<?php echo base_url().'logistic_in_use/get_seal_demo'?>', { value: string, barcode: barcode })
										.done(function( data ) {
											$("#bag_seal_cdm_<?=$index?>").textbox('setValue', data);
										}
									);
								}
								...
								if(string=='e') {
									$.post('<?php echo base_url().'logistic_in_use/get_seal_demo'?>', { value: string, barcode: barcode })
										.done(function( data ) {
											$("#bag_no_cdm_<?=$index?>").textbox('setValue', data);
										}
									);
								}
	...
	<?php
		if($row->denom=="100000") {
			$denom_values = $row->pcs_100000;
			$element = "#s100k_atm_$index";
		} else {
			$denom_values = $row->pcs_50000;
			$element = "#s50k_atm_$index";
		}
	?> 

<script type="text/javascript">
	function save1(target){
		var lembar = $("<?=$element?>").val();
		var denom_values = "<?=$denom_values?>";
		
		<?php 
			if($r['act']=="ATM") {
		?>
				if(denom_values==lembar) {
					var tr = $(target).closest('.datagrid-row-detail').closest('tr').prev();
					var index = parseInt(tr.attr('datagrid-row-index'));
					saveItem(index);
				} else {
					jq341.notify("Invalid, jumlah lembar", "error");
				}
		<?php 
			} else {
		?>

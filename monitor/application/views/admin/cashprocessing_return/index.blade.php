...
@section('content')
					...
					<tbody>
						...
						<tr>
							...
							<td style="text-align: center">
								<button type="button" class='button green' onClick="window.location.href='<?php echo base_url();?>cashprocessing_return/edit/<?php echo $row->id_ct;?>#tab-hcp'" title='Edit'><span class='smaller'>View Detail Data</span></button>
							</td>
						</tr>
						<?php 
							endforeach; 
	...
	<script>
		...
		var tables = jq3412('#example').DataTable({
			...
			lengthChange: false,
			ajax: {
				url: '<?=base_url()?>cashprocessing_return/json',
				data: function(data) {
					// Read values

...
class Cashprocessing_batal extends CI_Controller {
    var $data = array();
	...
	public function index() {
		$this->data['active_menu'] = "cashprocessing_batal";
	...
	public function add() {
        ...
        return view('admin/cashprocessing_batal/form', $this->data);
	}
	...
	public function show_form2() {
		...
		return view('admin/cashprocessing_batal/show_form', $this->data);
	...
	function update_data2x() {
		echo "AAA";
		
		$id = $this->input->get("id");
		$query = "SELECT client.denom FROM client LEFT JOIN cashtransit_detail ON(cashtransit_detail.id_bank=client.id) WHERE cashtransit_detail.id='$id'";
		$denom = json_decode($this->curl->simple_get(rest_api().'/select/query', array('query'=>$query), array(CURLOPT_BUFFERSIZE => 10)))->denom;
